...
long cTEXTURE::Allocate(long sizeX,long sizeY,long type)
{
	...
	if (Type==1)
	{
		...
		for (t=0;t<SizeY;t++)
		{
			for (i=0;i<SizeX;i++)
			{
				TexturePtr[(t*SizeX+i)*3]=0;
				TexturePtr[(t*SizeX+i)*3+1]=0;
				TexturePtr[(t*SizeX+i)*3+2]=0;
			}
		}
	}
...
long cTEXTURE::LoadTGA(char *filename)
{
	...
	file.open(filename,std::ios::in|std::ios::binary|std::ios::_Nocreate);
	...
	if (TGAHeader.Zero1!=0 || TGAHeader.Zero2!=0 ||
		...
		TGAHeader.val2!=0x18)
	{
		dale << "Error Loading Texture (only 24bit uncompressed supported)"<<std::endl;
		return (1==0);
	}
...
long cTEXTURE::LoadTGA()
{
	...
	file.open(Name,std::ios::in|std::ios::binary|std::ios::_Nocreate );
